...
{
  public:
    VirtualWallServer(): new_data_added(true)
    {
      ...
      //      if (!new_data_added) {
      //           debugCloud();
      //           new_data_added = false;  // Reset the flag after printing
      //       }
       r.sleep();
     }
...
{
    for (const auto& wall : req.walls)
    {
        // 检查是否已存在具有相同id的墙
        // auto it = std::find_if(clouds_.begin(), clouds_.end(), [&wall](const IDPointCloud& id_pc){ return id_pc.id == wall.id; });
        // if (it != clouds_.end()) {
        //     ROS_WARN("Wall with id %d already exists. Skipping.", wall.id);
        //     continue; // 或者更新现有的墙，或者返回错误
        // }
        deleteWallHandler(wall.id);

        IDPointCloud id_pc;
        id_pc.id = wall.id;
        id_pc.start_x = wall.start_point.x;
        id_pc.start_y = wall.start_point.y;
        id_pc.end_x = wall.end_point.x;
        id_pc.end_y = wall.end_point.y;
        
        // 假设createLineCloud总是成功的，如果不是，需要添加错误处理
        createLineCloud(wall.start_point.x, wall.end_point.x, wall.start_point.y, wall.end_point.y, id_pc.pc);

        clouds_.push_back(id_pc);
        new_data_added = true;  // Set the flag when new data is added
        debugCloud();
        ROS_INFO("Added wall with id %d", wall.id);
    }
    ...
    void createLineCloud(double wx0, double wx1, double wy0, double wy1, pcl::PointCloud < pcl::PointXYZ > &pc, double height = 0.2)
    {
